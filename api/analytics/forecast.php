...
try {
    ...
    $dailyProfitStmt->execute([$startKey, $endKey]);
    $dailyProfitRows = $dailyProfitStmt->fetchAll(PDO::FETCH_ASSOC);
    $dailyProfitMap = [];
    foreach ($dailyProfitRows as $row) {
        $dailyProfitMap[$row['date']] = (float)$row['profit'];
    }
    ...
    $profitSummaryStmt->execute([$startKey, $endKey]);
    $profitSummaryRow = $profitSummaryStmt->fetch(PDO::FETCH_ASSOC);

    $actualRevenue = (float)($profitSummaryRow['totalSales'] ?? 0.0);
    $actualCost = (float)($profitSummaryRow['totalCost'] ?? 0.0);
    $actualProfit = (float)($profitSummaryRow['totalProfit'] ?? 0.0);
    $actualMargin = $actualRevenue > 0 ? round(($actualProfit / $actualRevenue) * 100, 1) : 0.0;

    $forecastRevenue = array_reduce($forecastData, fn($carry, $entry) => $carry + ($entry['forecast'] ?? 0), 0.0);
    $forecastProfit = round($forecastRevenue * ($actualRevenue > 0 ? $actualProfit / $actualRevenue : 0.0), 2);
    $forecastCost = max(0.0, $forecastRevenue - $forecastProfit);
    $forecastMargin = $forecastRevenue > 0 ? round(($forecastProfit / $forecastRevenue) * 100, 1) : 0.0;

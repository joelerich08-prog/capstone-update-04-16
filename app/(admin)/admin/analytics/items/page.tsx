...
export default function ItemsAnalyticsPage() {
  ...
  return (
    <DashboardShell
      title="Sales by Item"
      description="Analyze product performance, inventory status, and profitability trends"
      allowedRoles={['admin']}
    >
      ...
      <ProfitabilitySummary
        ...
        description="Profitability at a product level for the selected range"
      />
      ...
      <Card className="mt-4">
        <CardHeader>
          ...
          <CardDescription>Items with the smallest profit margin this period</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {lowMarginItems.map(item => (
                <Card key={item.productId} className="border">
                  <CardContent>
                    ...
                    <p className="text-xl font-bold text-destructive">{item.profitMargin.toFixed(1)}%</p>
                    <p className="text-xs text-muted-foreground">Revenue {formatCurrency(item.revenue)}</p>
                    <p className="text-xs text-muted-foreground">Profit {formatCurrency(item.profit)}</p>
                  </CardContent>
                </Card>
              ))}
            </div>
          ) : (
            <p className="text-sm text-muted-foreground">No low-margin items found for the selected period.</p>
          )}
        </CardContent>
      </Card>

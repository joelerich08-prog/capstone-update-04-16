...
export default function OrdersPage() {
  const { orders, addOrder: addOrderToContext, updateOrderStatus: updateOrderStatusContext, validationError } = useOrders()
  ...
  const handleAddItem = () => {
    ...
    const basePrice = Number(selectedPriceType === 'wholesale' ? product.wholesalePrice : product.retailPrice)
    const adjustmentPrice = (selectedPriceType === 'retail' && variant && variant.priceAdjustment != null) ? Number(variant.priceAdjustment) : 0
    const unitPrice = basePrice + adjustmentPrice
  ...
  const calculateTotal = () => {
    return orderItems.reduce((sum, item) => sum + Number(item.unitPrice) * Number(item.quantity), 0)
  }
  ...
  const handleCreateOrder = async () => {
    ...
    if (!newOrder) {
      toast.error(validationError || 'Failed to create order')
      return
    }
      ...
      <Dialog open={isAddOrderOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateOrder} disabled={!customerName.trim() || !customerPhone.trim() || orderItems.length === 0}>
              Create Order
            </Button>
          </DialogFooter>
        </DialogContent>
